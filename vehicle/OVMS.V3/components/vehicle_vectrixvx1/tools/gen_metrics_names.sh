...
for f in $CMOD_METRICS ; do
  fu=${f//\./_}
  printf 'const char* const x_vv_b_cmod_%s[] = { ' "$fu"
  sep=""
  for ((i = 0; i < $CMOD_CNT; i++)) ; do
    ih=$((i + 1))
    printf '%s"xvv.b.m.%02d.%s"' "$sep" "$ih" "$f"
    sep=", "
  done
  echo ' };'
done
...
for f in $CELL_METRICS ; do
  fu=${f//\./_}
  printf 'const char* const x_vv_b_cell_%s[] = { ' "$fu"
  sep=""
  for ((i = 0; i < $CELL_CNT; i++)) ; do
    ih=$((i + 1))
    printf '%s"xvv.b.c.%02d.%s"' "$sep" "$ih" "$f"
    sep=", "
  done
  echo ' };'
done
...
for f in $SPEED_METRICS ; do
  fu=${f//\./_}
  printf 'const char* const x_vv_p_stats_speed_%s[] = { ' "$fu"
  sep=""
  for ((i = 0; i < $SPEED_CNT; i++)) ; do
    ih=$((i + 1))
    printf '%s"xvv.p.stats.%s.%s"' "$sep" "${SPEED_NAMES[$i]}" "$f"
    sep=", "
  done
  echo ' };'
done
...
for f in $LEVEL_METRICS ; do
  fu=${f//\./_}
  printf 'const char* const x_vv_p_stats_level_%s[] = { ' "$fu"
  sep=""
  for ((i = 0; i < $LEVEL_CNT; i++)) ; do
    ih=$((i + 1))
    printf '%s"xvv.p.stats.%s.%s"' "$sep" "${LEVEL_NAMES[$i]}" "$f"
    sep=", "
  done
  echo ' };'
done
